...
class DynamicGestureApp(AIBase):
    ...
    def config_preprocess(self,input_image_size=None):
        with ScopedTiming("set preprocess config",self.debug_mode > 0):
            ...
            self.ai2d_resize.build(ai2d_input_size,[self.crop_params[0],self.crop_params[1]])
            self.ai2d_crop.crop(self.crop_params[2],self.crop_params[3],self.crop_params[4],self.crop_params[5])
            self.ai2d_crop.build([self.crop_params[0],self.crop_params[1]],self.model_input_size)
    ...
    # 重写预处理，因为该部分不是单纯的走ai2d做初始化，所以该函数需要重写
    def preprocess(self,input_np):
        resize_tensor=self.ai2d_resize.run(input_np)
        return self.ai2d_crop.run(resize_tensor.to_numpy())

    # run函数重写
    def run(self,input_np,his_logit,history):
        ai2d_output_tensor=self.preprocess(input_np)
        ai2d_output = ai2d_output_tensor.to_numpy()
