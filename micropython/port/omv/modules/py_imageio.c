...
STATIC mp_obj_t py_imageio_read(size_t n_args, const mp_obj_t *args, mp_map_t *kw_args) {
    ...
    if (arg_other) {
        ...
        image.alloc_type = arg_other->alloc_type;
        image.cache = arg_other->cache;
        image.phy_addr = arg_other->phy_addr;
        image.pool_id = arg_other->pool_id;
        image.ref_obj = arg_other->ref_obj;
    } else {
