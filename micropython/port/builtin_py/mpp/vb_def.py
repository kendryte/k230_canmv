...
vb_block_info_desc = {
    "virt_addr": 0 | uctypes.UINT64,
    "phys_addr": 8 | uctypes.UINT64,
    "handle": 16 | uctypes.UINT32,
    "pool_id": 20 | uctypes.UINT32,
    "size": 24 | uctypes.UINT32,
}

def vb_block_info_parse(s, kwargs):
    s.virt_addr = kwargs.get("virt_addr", 0)
    s.phys_addr = kwargs.get("phys_addr", 0)
    s.pool_id = kwargs.get("pool_id", 0)
    s.handle = kwargs.get("handle", 0)
    s.size = kwargs.get("size", 0)
