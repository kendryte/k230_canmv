...
static void sdhci_send_command(struct sdhci_host *sdhci_host, struct sdhci_command *command, rt_bool_t enDMA)
{
    ...
    uint32_t cmd_r = 0, xfer_mode = 0;
    struct sdhci_data *sdhci_data = sdhci_host->sdhci_data;

    cmd_r = sdhci_readw(sdhci_host, SDHCI_COMMAND);
    xfer_mode = sdhci_readw(sdhci_host, SDHCI_TRANSFER_MODE);
    /* config cmd_r and xfer_mode */
    cmd_r &= ~0xffff;
...
static rt_err_t sdhci_transfer_data_blocking(struct sdhci_host *sdhci_host, struct sdhci_data *data, rt_bool_t enDMA)
{
    ...
    if (enDMA)
    {
        ...
        while (!(sdhci_get_int_status_flag(sdhci_host) &
                 (sdhci_data_complete_flag | sdhci_data_error_flag | sdhci_dma_error_flag | sdhci_tuning_error_flag)))
        {
        }
    ...
    else
    {
        ...
        sdhci_writeb(sdhci_host, ctrl, SDHCI_HOST_CONTROL);
#endif
        ...
        do {
            ...
            if (!transfer_done && (stat & SDHCI_INT_DMA_END))
            {
                ...
                if (data->rxData)
                {
                    start_addr = (rt_ubase_t)((uint8_t *)data->rxData + PV_OFFSET);
                ...
                {
                    start_addr = (rt_ubase_t)((uint8_t *)data->txData + PV_OFFSET);
                    sdhci_writel(sdhci_host, SDHCI_INT_DMA_END, SDHCI_INT_STATUS);
                    start_addr &= ~(SDHCI_DEFAULT_BOUNDARY_SIZE - 1);
                    start_addr += SDHCI_DEFAULT_BOUNDARY_SIZE;
                    sdhci_writel(sdhci_host, start_addr, SDHCI_DMA_ADDRESS);
...
#if defined(SDHCI_ENABLE_CACHE_CONTROL)
    /* invalidate cache for read */
    // if ((data != RT_NULL) && (data->rxData != RT_NULL))
    // {
    //     /* invalidate the DCACHE */
    //     rt_hw_dsb();
    //     rt_hw_cpu_dcache_ops(RT_HW_CACHE_INVALIDATE, (void *)data->rxData, (data->blockSize) * (data->blockCount));
    // }
#endif
    return error;
}
...
rt_err_t sdhci_set_transfer_config(struct sdhci_host *sdhci_host, struct sdhci_command *sdhci_command, struct sdhci_data *sdhci_data)
{
    ...
    switch (sdhci_command->responseType)
    {
        ...
        case card_response_type_r5b: /* Response 5 with busy */
            sdhci_command->flags |=
                (sdhci_cmd_resp_short_busy | sdhci_enable_cmd_crc_flag | sdhci_enable_cmd_index_chk_flag);
            break;
...
static rt_err_t sdhci_receive_command_response(struct sdhci_host *sdhci_host, struct sdhci_command *command)
{
    ...
    if ((command->responseErrorFlags != 0U) &&
        ((command->responseType == card_response_type_r1) || (command->responseType == card_response_type_r1b) ||
         (command->responseType == card_response_type_r6) || (command->responseType == card_response_type_r5)))
    {
...
#if defined(SDHCI_ENABLE_CACHE_CONTROL)
    if ((sdhci_data != RT_NULL) && (!(sdhci_data->executeTuning)))
    {
        if (sdhci_data->txData != RT_NULL)
        {
            /* clear the DCACHE */
            rt_hw_cpu_dcache_ops(RT_HW_CACHE_FLUSH, (void *)sdhci_data->txData, (sdhci_data->blockSize) * (sdhci_data->blockCount));
        }
        else
        {
            /* clear the DCACHE */
            rt_hw_cpu_dcache_ops(RT_HW_CACHE_FLUSH, (void *)sdhci_data->rxData, (sdhci_data->blockSize) * (sdhci_data->blockCount));
            rt_hw_cpu_dcache_ops(RT_HW_CACHE_INVALIDATE, (void *)sdhci_data->rxData, (sdhci_data->blockSize) * (sdhci_data->blockCount));
        }
    }
#endif
