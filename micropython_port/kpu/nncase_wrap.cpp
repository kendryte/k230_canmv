...
{
    // std::cout << "C++: in set input tensor"<< std::endl;
    runtime_tensor *copy = new runtime_tensor();
    copy->r_tensor = new nncase::runtime::runtime_tensor();
    // std::cout << "C++: start to copy"<< std::endl;
    *(copy->r_tensor) = *(tensor->r_tensor); 
    // auto a = new nncase::runtime::runtime_tensor(tensor->r_tensor->impl());
    // std::cout<<"C++: set input tensor\n"<<std::endl;
    p->interp->input_tensor(index, *copy->r_tensor).expect("kpu set input tensor failed.");
}
...
runtime_tensor* Kpu_get_input_tensor(Kpu* p, size_t index)
{
    ...
    // std::cout<<"end copy, set to result tensor"<<std::endl;
...
{
    // runtime_tensor *tensor = new runtime_tensor();
    // nncase::runtime::runtime_tensor out = p->interp->output_tensor(index).expect("kpu get output tensor failed.");
    // *tensor->r_tensor = std::move(out);
    // return tensor;
    runtime_tensor *tensor = new runtime_tensor();
    auto data = p->interp->output_tensor(index).expect("kpu get input tensor failed.");
    // std::cout << "C++: start copy" << std::endl;
    tensor->r_tensor = new nncase::runtime::runtime_tensor(data.impl());
    // std::cout<<"C++: end copy, set to result tensor"<<std::endl;
...
runtime_tensor* from_numpy(int dtype, finite_data shape, void* data, uint64_t phy_addr)
{
    ...
    for (int i = 0; i < shape.data_size; i++)
    {
        ...
        // std::cout << shape_data[i] << std::endl;
    }
    
    // std::cout<<"from_numpy"<<std::endl;
    // auto alldata = std::accumulate(shape_data.begin(), shape_data.end(), 1, std::multiplies<int32_t>());
    ...
    // std::cout<<"phy_addr: "<<phy_addr<<std::endl;
    ...
    // std::cout << " from numpy ok "<< std::endl;
    return tensor;
}

void to_numpy(runtime_tensor * tensor, rt_to_ndarray_info *info)
{
    // std::cout << "start to numpy" << std::endl;
    ...
    {
        // std::cout <<i<<": "<< shape[i] <<", "<< tensor->r_tensor->strides()[i] << std::endl;
    ...
    // auto alldata = std::accumulate(shape.begin(), shape.end(), 1, std::multiplies<int32_t>());
    // for (int i = 0; i < alldata; i++)
    // {
    //     if(i % 4 == 0)
    //         std::cout<<std::endl;
    //     std::cout<<(int)*((uint8_t*)info->data_+i)<<" ";
        
    // }
    // std::cout << "end get data" << std::endl;
    // return info;
}
...
template<class T>
void print_vector(std::vector<T> data, std::string name)
{
...
void print_vector(nncase::dims_t data, std::string name)
{
    std::cout << name << std::flush;
    for (auto i : data)
        std::cout << i << "," << std::flush;
...
{
    // std::cout << "C++: in ai2d build" << std::endl;
    // nncase::dims_t in_shape = nncase::dims_t((size_t *)input_shape.data, (size_t *)input_shape.data + input_shape.data_size);
    // nncase::dims_t out_shape = nncase::dims_t((size_t *)output_shape.data, (size_t *)output_shape.data + output_shape.data_size);
    ...
    // std::cout << "in_shape :";
    // for (auto i : in_shape_)
    // {
    //     std::cout << i << ",";
    // }
    // std::cout << std::endl<<"out_shape :";
    // for (auto i : out_shape_)
    // {
    //     std::cout << i << ",";
    // }
    // std::cout << std::endl;
    ...
    if(in_shape_[3]<=32 && p->ai2d_pad_param.paddings[3].before>0)
        throw std::runtime_error("ai2d pad: input width is <=32, the left pad should not be set. You can set the right pad first, then set the left pad.");
    // std::cout << "C++: start new builder" << std::endl;
    m_builder *mbuilder = new m_builder;
    // std::cout << "C++: end new builder" << std::endl;
    mbuilder->builder = new nncase::F::k230::ai2d_builder(in_shape,
                                                     ...
                                                     p->ai2d_resize_param,
                                                     p->ai2d_affine_param);
    // std::cout << "crop :" << p->ai2d_crop_param.crop_flag << std::endl;
    // std::cout << "shift :" << p->ai2d_shift_param.shift_flag << std::endl;
    // std::cout << "pad :" << p->ai2d_pad_param.pad_flag << std::endl;
    // std::cout << "resize :" << p->ai2d_resize_param.resize_flag << std::endl;
    // std::cout << "affine :" << p->ai2d_affine_param.affine_flag << std::endl;
    
    // std::cout << "C++: before build schedule" << std::endl;
    // builder_.dump_asm_ = false;
    // builder_.build_schedule();
    // std::cout << "C++: finish new builder" << std::endl;
    // mbuilder->builder = builder;
    // std::cout << "C++: start build schedule" << std::endl;
    mbuilder->builder->build_schedule().expect("ai2d build schedule failed.");
    // std::cout << "C++: end build schedule" << std::endl;
    return mbuilder;
}

void ai2d_run(m_builder* p, runtime_tensor *in_tensor, runtime_tensor *out_tensor)
{
    // std::cout<<"C++: before run" << std::endl;

    auto s1 = in_tensor->r_tensor->shape();
    auto st1 = in_tensor->r_tensor->strides();
    // for(auto &i: s1)
    // {
    //     std::cout<<i<<" ";
    // }
    ...
    runtime_tensor *copy = new runtime_tensor();
    copy->r_tensor = new nncase::runtime::runtime_tensor();
    *(copy->r_tensor) = *(in_tensor->r_tensor); 
    
    runtime_tensor *copy2 = new runtime_tensor();
    copy2->r_tensor = new nncase::runtime::runtime_tensor();
    *(copy2->r_tensor) = *(out_tensor->r_tensor); 
    
    // auto s = copy->r_tensor->shape();
    // auto st = copy->r_tensor->strides();
    // for(auto &i: s)
    // {
    //     std::cout<<i<<" ";
    // }
    ...
    //     std::cout<<i<<" ";
    // }
    p->builder->invoke(*copy->r_tensor, *copy2->r_tensor).expect("ai2d run failed.");
    // std::cout<<"C++: after run" << std::endl;
}
...
void ai2d_set_pad_param(ai2d *p, ai2d_pad_param pad_params)
{
    ...
    // std::cout<<"paddings_index: "<<std::endl;
    // for(auto i :paddings_index)
    // {
    //     std::cout<<i<<", "<<std::endl;
    // }
    ...
    // for (auto i : p->ai2d_pad_param.pad_val)
    // {
    //         std::cout << i << ", " << std::endl;
    // }
