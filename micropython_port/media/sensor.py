...
if BOARD_NAME == "k230d_canmv":
    CAM_DEFAULT_SENSOR = OV_OV5647_MIPI_1920X1080_30FPS_10BIT_LINEAR
    CAM_DEFAULT_OUTPUT_BUF_NUM = 4
elif BOARD_NAME == "k230_canmv_01studio":
    CAM_DEFAULT_SENSOR = OV_OV5647_MIPI_CSI2_1920X1080_30FPS_10BIT_LINEAR_V2

# NOTE:This is a private class for internal use only!!!
#      Don't edit it arbitrarily!!!

class __sensor_device:
    def __init__(self):
...
class sensor:
    ...
    cam_dev = [__sensor_device() for i in range(0, VICAP_DEV_ID_MAX)]
    YUV420SP = PIXEL_FORMAT_YUV_SEMIPLANAR_420
    RGB888   = PIXEL_FORMAT_RGB_888
    RGB888P  = PIXEL_FORMAT_RGB_888_PLANAR
    ...
    # set_outfmt
    @classmethod
    def set_pixformat(cls, dev_num, chn_num, pix_format):
        if (dev_num > CAM_DEV_ID_MAX - 1) or (chn_num > CAM_CHN_ID_MAX - 1):
            raise ValueError(f"invalid param, dev_num({dev_num}, chn_num({chn_num}))")
    ...
    # capture_image
    @classmethod
    def snapshot(cls, dev_num, chn_num):
    ...
    # release_image
    @classmethod
    def unsnapshot(cls, dev_num, chn_num, img):
